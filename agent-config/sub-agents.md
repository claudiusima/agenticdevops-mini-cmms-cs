...
- **A dedicated QA sub-agent** — that is QA's job, and QA belongs in the **review agent** (`cursor-rules.template.md`), which can actually *run* the app. A coding-agent sub-agent runs in the coding agent's own sandbox and would just re-do the PM's static read. Put QA where it can execute rather than spawn an agent that can only re-read.
